...
class StockPickingType(models.Model):
    ...
    def importar_productos_desde_api(self):
        ...
        try:
            ...
            # 2. Procesar productos y precios
    ...
    def _obtener_datos_api(self, url, headers):
        try:
            ...
            if isinstance(data, dict):
                return data.get('datos', data.get('data', data))
    ...
    def _procesar_productos(self, productos_data, precios_data):
        ...
        # Normalizar y organizar precios por KOPR
        precios_por_kopr = {}
        for item in precios_data:
            if not isinstance(item, dict) or item.get('error'):
                continue
            kopr = (item.get('kopr') or '').strip().upper()
            if not kopr:
                continue
        ...
        # Procesar productos con precios
        for item in productos_data:
            ...
            if kopr in precios_por_kopr:
                ...
                _logger.warning("No se encontró precio válido para KOPR: %s", kopr)
